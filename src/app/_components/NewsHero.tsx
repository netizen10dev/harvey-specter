...
export default function NewsHero() {
  ...
  useEffect(() => {
    ...
    const splitL = root.querySelector<HTMLElement>("[data-news-split-l]");
    const splitR = root.querySelector<HTMLElement>("[data-news-split-r]");
    const lines  = root.querySelectorAll<HTMLElement>("[data-news-line]");
    const label  = root.querySelector<HTMLElement>("[data-news-label]");
    const bg     = root.querySelector<HTMLElement>("[data-news-bg]");
  ...
  return (
    <div ref={wrapperRef} className="relative min-h-screen w-full overflow-hidden bg-white">
      <div data-news-bg="" className="pointer-events-none absolute inset-0 origin-center bg-[#f0f0f0]" />
      <div className="relative z-10 flex min-h-screen flex-col items-center justify-center px-4 text-center md:px-8">
        ...
        <h1 className="font-medium capitalize tracking-[-0.07em] leading-[0.88] text-black text-[clamp(56px,10vw,148px)]">
          <span data-news-line="" data-news-split-l="" className="block">What&rsquo;s</span>
          <span data-news-line="" className="block">new in</span>
          <span data-news-line="" data-news-split-r="" className="block">studio.</span>
        </h1>
      </div>
    </div>
  );
