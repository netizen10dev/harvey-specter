...
export default function ProjectsHero() {
  ...
  useEffect(() => {
    ...
    const splitL = root.querySelector<HTMLElement>("[data-prj-split-l]");
    const splitR = root.querySelector<HTMLElement>("[data-prj-split-r]");
    const lines  = root.querySelectorAll<HTMLElement>("[data-prj-line]");
    const label  = root.querySelector<HTMLElement>("[data-prj-label]");
    const bg     = root.querySelector<HTMLElement>("[data-prj-bg]");
    ...
    >
      ...
      <div className="relative z-10 flex min-h-screen flex-col items-center justify-center px-4 text-center md:px-8">
        ...
        <h1 className="font-medium capitalize tracking-[-0.07em] leading-[0.88] text-black text-[clamp(56px,10vw,148px)]">
          <span data-prj-line="" data-prj-split-l="" className="block">Projects</span>
          <span data-prj-line="" className="block">that speak</span>
          <span data-prj-line="" data-prj-split-r="" className="block">for themselves.</span>
        </h1>
      </div>
    </div>
  );
